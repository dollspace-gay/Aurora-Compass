...
use atproto_client::xrpc::{
    XrpcClient, XrpcClientConfig, XrpcError, XrpcRequest, XrpcResponse,
};
...
async fn test_request_with_custom_headers() {
    ...
    Mock::given(method("GET"))
        .and(path("/xrpc/com.example.getQuery"))
        .respond_with(ResponseTemplate::new(200).set_body_json(&TestQuery {
            name: "test".to_string(),
            value: 1,
        }))
        .mount(&mock_server)
...
async fn test_default_headers() {
    ...
    let client = XrpcClient::new(config);
    assert_eq!(
        client.config().default_headers.get("X-API-Key"),
